...
def process_todos_to_single_issue(*, client: GitHubClient, issues: list[Issue]):
    ...
    # TODO
    ## Active
    * [ ] [path_to_file:line](githuburl_with_hash_and_line) my whatever
    * [ ] my holalala
    * [ ] #3
    ## Done
    * [x] ~~asdasd~~
    * [x] #2
    """
    FOOTER = "_Generated by Github Action, please do not edit. If you want to extract TODO into sub-issue, " \
             "please include `file:line` somethere in the body text and add label `todo`._"
    title = os.getenv('INPUT_TARGET_ISSUE_TITLE')
    print(f'Process all {len(issues)} todos into one issue: {title}.')

    target: GithubIssue

    for issue in client.existing_issues:
    ...
    done_todo_lines = lines[done_heading_index + 1:footer_index]
    ...
    print('Titles to lines:', active_titles_to_lines)

    for found_todo in issues_to_process:
        print(f'Processing {found_todo.title} {found_todo.file_name} {found_todo.status}')
    ...
    update_request = requests.patch(
        ...
        data=json.dumps(dict(
            body="# TODO\n## Active\n{}\n## Done\n{}\n\n---\\{}".format(
                ...
                '\n'.join(sorted(set(done_todo_lines))),
                FOOTER,
            )
        ))
    )
    print("Issue updated, response:")
    print(update_request.json())
    assert update_request.ok, "Updated issue with new content"
